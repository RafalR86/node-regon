...
var Transport = function (wsdl, options) {
  this.getClient = function () {
    var wsdlOptions = {
      "forceSoap12Headers": true,
      "escapeXML": false
    }
    return soap.createClientAsync(wsdl, wsdlOptions);
  }
  ...
  this.getParams = function (function_name, value, type, silosId) {
    ...
    if (type == "F") {
      if (type == "F"){
        if(silosId == 1) {
          ...
          raportName = 'PublDaneRaportFizycznaOsoba';
        }
      } else if (type == "P"){
    ...
    switch (function_name) {
      ...
      case 'DaneSzukaj':
        ...
        break;
      case 'DaneSzukajPodmioty':
        response = '<tns:DaneSzukajPodmioty><tns:pParametryWyszukiwania>' + value + '</tns:pParametryWyszukiwania></tns:DaneSzukajPodmioty>';
        break;
    }
    return response;
  }
